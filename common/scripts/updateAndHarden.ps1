...
Foreach ($InsecureHash in $InsecureHashes) {
  $key = (Get-Item HKLM:\).OpenSubKey('SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes', $true).CreateSubKey($InsecureHash)
  New-ItemProperty -path "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Hashes\$InsecureHash" -name 'Enabled' -value '0' -PropertyType 'DWord' -Force | Out-Null
  $key.close()
  Write-Host "Hash $InsecureHash has been disabled."
}
...
# Force SMB signing on both Client and Server
# Client
...
Write-Host "SMB Secure Client Signing has been enabled."
...
Write-Host "SMB Secure Server Signing has been enabled"
...
# THIS CAN BE VERY DANGEROUS - TEST TEST TEST!
###########################################
...
Write-Host '$defaultSecureProtocolsSum = ' $defaultSecureProtocolsSum

#######################################
# Remote Desktop Services: Enable NLA Requirement
#######################################
Write-Host 'Enable NLA for Remote Desktop connections' 
...
Write-Host 'Require high level encryption for Remote Desktop sessions'
New-ItemProperty -path "HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" -name 'MinEncyptionLevel' -value 3 -PropertyType 'DWord' -Force | Out-Null

Write-Host 'A computer restart is required to apply settings. Will restart after OS updates.'
